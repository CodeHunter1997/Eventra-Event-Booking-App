...
import com.example.eventapp.User.Authentication.LoginViewModel
...
class OrganizerLogin : AppCompatActivity() {
    private val loginViewModel: OrganizerLoginViewModel by viewModels()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        forgotPassword.setOnClickListener {
            val intent = Intent(this, OrganizerForgotPassword::class.java)
            startActivity(intent)
        }
        ...
        loginBtn.setOnClickListener {
            ...
            val request = OrganizerLoginDataModelRequest(
                email = email,
                password = password
            )
            ApiClient.instance.loginOrganizer(request).enqueue(object : Callback<OrganizerLoginResponse> {
                override fun onResponse(call: Call<OrganizerLoginResponse?>, response: Response<OrganizerLoginResponse?>) {
                    if (response.isSuccessful) {
                        ...
                        if (!token.isNullOrEmpty()) {
                            ...
                            with(sharedPref.edit()) {
                                ...
                                putString("userType", "organizer") // <-- ADD THIS LINE

                                apply()
                            }
                        }
                        Toast.makeText(applicationContext, loginData?.message ?: "Login successful", Toast.LENGTH_LONG).show()
                        startActivity(Intent(this@OrganizerLogin, OrganizerHomePage::class.java))
                        finish()
                    }
                    else{
                        Toast.makeText(applicationContext, "Login failed: ${response.code()}", Toast.LENGTH_LONG).show()
                    }
                }
                override fun onFailure(call: Call<OrganizerLoginResponse?>, t: Throwable) {
